...
class sfImageThumbnailGeneric extends sfImageTransformAbstract
{
     ...
     */
    protected $methods = ['fit', 'scale', 'inflate','deflate', 'left' ,'right', 'top', 'bottom', 'center', 'top-left', 'top-right', 'bottom-left', 'bottom-right'];
    ...
    public function setMethod($method)
    {
        ...
        $map = ['left' => 'west', 'right' => 'east', 'top' => 'north', 'bottom' => 'south'];
        if ($key = array_search($method, $map)) {
            $message = sprintf('sfImageTransformPlugin thumbnail method \'%s\' is depreciated use \'%s\'', $method, $key);
            sfContext::getInstance()->getEventDispatcher()->notify(new sfEvent($this, 'application.log', [$message, 'priority' => sfLogger::ERR]));
        }
