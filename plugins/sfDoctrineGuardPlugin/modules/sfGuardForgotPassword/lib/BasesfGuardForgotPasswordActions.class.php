...
abstract class BasesfGuardForgotPasswordActions extends sfActions
{
    ...
    protected function sendRequestMail($user, $forgotPassword)
    {
        ...
        $message = $this->getMailer()->compose(
            ...
            $user->email_address,
            $i18n->__('Forgot Password Request for %name%', ['%name%' => $user->username], 'sf_guard'),
            $this->getPartial('sfGuardForgotPassword/send_request', ['user' => $user, 'forgot_password' => $forgotPassword]),
    ...
    protected function sendChangeMail($user, $password)
    {
        ...
        $message = $this->getMailer()->compose(
            sfConfig::get('app_sf_guard_plugin_default_from_email', 'from@noreply.com'),
            $user->email_address,
            $i18n->__('New Password for %name%', ['%name%' => $user->username], 'sf_guard'),
            $this->getPartial('sfGuardForgotPassword/new_password', ['user' => $user, 'password' => $password]),
        )->setContentType('text/html');
