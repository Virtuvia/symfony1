    ...
    protected function doClean($value)
    {
        ...
        if ($username) {
            if ($callable = sfConfig::get('app_sf_guard_plugin_retrieve_by_username_callable')) {
                $user = call_user_func_array($callable, [$username]);
            } else {
            ...
            if ($user) {
                ...
                if ($user->getIsActive() && $user->checkPassword($password)) {
                    return array_merge($value, ['user' => $user]);
                }
            }
        }
        ...
        throw new sfValidatorErrorSchema($this, [$this->getOption('username_field') => new sfValidatorError($this, 'invalid')]);
    }
