<?php

/**
 * ##COPYRIGHT PLACEHOLDER##
 */

/**
 
 * BaseFormGeneratorTest
 *
 * This class has been auto-generated by the Doctrine ORM Framework
 *
 * @property string $name
 * @property string $id
 *
 * @method string            getName($load = true) Returns the current record's "name" value
 * @method string            getId($load = true) Returns the current record's "id" value
 * @method FormGeneratorTest setName($value, $load = true) Sets the current record's "name" value
 * @method FormGeneratorTest setId($value, $load = true) Sets the current record's "id" value
 *
 */
abstract class BaseFormGeneratorTest extends myDoctrineRecord
{

    public function setTableDefinition()
    {
        $this->setTableName('form_generator_test');
        $this->hasColumn('name', 'string', 255, array(
             'type' => 'string',
             'length' => 255,
             ));

        $this->option('symfony', array(
             'form' => true,
             'filter' => false,
             ));
    }

    public function setUp()
    {
        parent::setUp();
        
    }

}