...
$t->is('text/html; charset=ISO-8859-1', $response->getHttpHeader('content-type'), '->setContentType() overrides previous content type if replace is true');
...
$t->is(array_keys($response->getStylesheets()), ['first', 'test', 'bar', 'last'], '->getStylesheets() removes a stylesheet from the response');

$response->removeStylesheet('first');
$t->is(array_keys($response->getStylesheets()), ['test', 'bar', 'last'], '->getStylesheets() removes a stylesheet from the response');
...
$t->is($response->getCookies(), ['foo' => ['name' => 'foo', 'value' => 'bar', 'expire' => null, 'path' => '/', 'domain' => '', 'secure' => true, 'httpOnly' => true, 'sameSite' => 'Lax']], '->setCookie() adds a cookie for the response');
